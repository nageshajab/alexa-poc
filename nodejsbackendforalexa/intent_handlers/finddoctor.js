...
const { finddoctor } = require("../HelperClasses/CsvHelper");
  ...
  async handle(handlerInput) {
    ...
    return handlerInput.responseBuilder
      .speak(
        "location is " +
          location +
          ". Please tell , which specialty doctor are you looking for"
      )
...
const FindDoctorBySpecialty = {
  ...
  async handle(handlerInput) {
    ...
    let specialty =
      handlerInput.requestEnvelope.request.intent.slots.specialty.value;
...
function getLocation(handlerInput) {
  ...
  if (location == null) {
    if (handlerInput.requestEnvelope.request.intent.slots != undefined) {
      if (
        handlerInput.requestEnvelope.request.intent.slots.location != undefined
      ) {
        location =
          handlerInput.requestEnvelope.request.intent.slots.location.value;
      }
